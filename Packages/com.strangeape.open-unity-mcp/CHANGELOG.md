...
- Add bulk scene-property editing with reload-marker validation, complete preflight, grouped Undo, and integrated final-value readback.
- Install a shared companion skill alongside Codex and Claude Code configuration, using project-local agent-specific paths and preserving customized files.
- Add compilation-idle waits inside the sidecar and checkpoint receipt metadata under Temp for interrupted-operation diagnosis.
- Stop treating delayed response headers as proof that a mutation was never sent; uncertain requests are not replayed, including failures during recovery retries.
- Add Node session/transport CI and a live Unity-sidecar SDK integration test.
...
- Fixed `unity.execute_csharp` failing on macOS with `Could not find Unity's C# compiler.` Compiler and Mono runtime paths were derived from `EditorApplication.applicationPath` plus a `Data` folder, which only matches the Windows/Linux editor layout â€” on macOS the application path is the `Unity.app` bundle itself and MonoBleedingEdge lives under `Contents/Resources/Scripting`. Paths are now derived from `EditorApplication.applicationContentsPath` and both layouts are probed, so `execute_csharp` resolves the compiler on every platform. Thanks @jhusting (#1).
...
- The required flag and token are snapshotted on the main thread when the server starts (never read from `EditorPrefs` on the accept-loop threads), so a settings or token change takes effect on the **next server restart** â€” noted in the Preferences UI.
...
- Added a per-tool `runOnCallerThread` opt-out in the tool registry so a tool can run its body on the caller thread instead of the main-thread pump; the enabled-check still marshals onto the main thread to read `EditorPrefs`, and the `try/catch â†’ "Tool failed: â€¦"` behavior is identical to main-thread tools. Only `unity.execute_csharp` opts in.
...
- Fixed a main-thread dispatch race where a timed-out tool call could still execute later â€” double-executing retried requests and signaling a disposed wait handle; timed-out work is now cancelled before it runs.
